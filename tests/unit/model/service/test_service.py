...
def test_calculate_loan(mock_uow):
    ...
    mock_uow.loan.get_loan.return_value = Loan(total_loan=100000, tenor=5, down_payment=25000)
    mock_uow.loan.get_vehicle.return_value = Vehicle(vehicle_type="mobil", vehicle_condition="Baru", vehicle_year=2019)
    ...
    assert math.isclose(loan_output_list[0].installment, 6583.333333, rel_tol=1e-6)
...
def test_input_loan(mock_uow):
    ...
    mock_uow.loan.add_loan.assert_called_once_with(Loan(total_loan=100000, tenor=5, down_payment=25000))
...
def test_input_vehicle(mock_uow):
    ...
    input_service.input_vehicle(vehicle_type="mobil", vehicle_condition="Baru", vehicle_year=2019)

    # Perform assertions on the mock UoW's add_vehicle method
    mock_uow.loan.add_vehicle.assert_called_once_with(Vehicle(vehicle_type="mobil", vehicle_condition="Baru", vehicle_year=2019))
