...
class LoanController:
    ...
    def run_menu(self):
        ...
        if menu == "1":
            ...
            self.run_calculation_from_api(url='http://www.mocky.io/v2/5d06e6ae3000005300051d16')
    def run_input_calculation(self):
        ...
        if vehicle_condition == "baru" and vehicle_year < int(datetime.date.today().year - 1):
            while vehicle_condition == "baru" and vehicle_year < int(datetime.date.today().year - 1):
                self.view.output("Tahun kendaraan baru tidak boleh lebih dari 1 tahun")
                vehicle_year = self.view.input_vehicle_year()
    ...
    def run_calculation_from_file(self, filepath):
        ...
        self.loan_input_service = LoanInputService(uow=self.uow)
        self.loan_input_service.input_vehicle(vehicle_type=json_data['vehicleType'].lower(),
                                              vehicle_condition=json_data['vehicleCondition'].lower(),
                                              vehicle_year=json_data['tahunMobil'])
        self.loan_input_service.input_loan(total_loan=float(json_data['jumlahPinjaman']),
    ...
    def run_calculation_from_api(self, url):
        ...
        stripped_data = stripped_data.replace('"responseMessage":"Succeed"', '')
        json_data = json.loads(stripped_data)
        json_data = json_data['vehicleModel']
        self.loan_input_service = LoanInputService(uow=self.uow)
        self.loan_input_service.input_vehicle(vehicle_type=json_data['vehicleType'].lower(), vehicle_condition=json_data['vehicleCondition'].lower(), vehicle_year=json_data['tahunMobil'])
        self.loan_input_service.input_loan(total_loan=float(json_data['jumlahPinjaman']), tenor=int(json_data['tenorCicilan']), down_payment=float(json_data['jumlahDownPayment']))
        self.loan_service = LoanService(uow=self.uow)
        calculation = self.loan_service.calculate_loan()
        self.view.output_loan(calculation)
        self.run_menu()
